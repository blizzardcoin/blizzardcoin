...
uint256 hashGenesisBlock("0x15fe92c013874a0eb1ba3dd6cff4fbbe78c8cc3cf646101a8e49cdf6e9b878f7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Blizzardcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Blizzardcoin Signed Message:\n";
...
{
    // Blizzardcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Blizzardcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 300; // Blizzardcoin: 5 minutes
static const int64 nTargetSpacing = 2.33 * 60; // Blizzardcoin: 2.33 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blizzardcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blizzardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Blizzardcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Wikipedia 26/Mar/2014 Russia formally annexes Crimea while the European Union and Ukraine sign an association agreement.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1395877681;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 0;

        if (fTestNet)
        {
            block.nTime    = 1395864360;
        ...
        assert(block.hashMerkleRoot == uint256("0xe786a4826ee6fb44ccd9d82a1425ff6fcb6c30b69cf5181a23aa9fee7bb6ad7c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Blizzardcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlizzardcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlizzardcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlizzardcoinMiner(CWallet *pwallet)
{
    printf("BlizzardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blizzardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BlizzardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlizzardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlizzardcoinMiner, pwallet));
}
